...
QVariant LoginModel::data(const QModelIndex& index, int role) const
{
	...
	case Image:
		return QVariant(mData[index.row()].image == "" ? mData[index.row()].identicon
													   : mData[index.row()].image);
	case Name:
		return QVariant(mData[index.row()].name);
	case PublicKey:
		return QVariant(mData[index.row()].keyUid);
	}
...
QJsonObject loginAccount(NodeAccount* nodeAccount, QString password)
{
	QString hashedPassword = "0x" + QString::fromUtf8(
		QCryptographicHash::hash(password.toUtf8(), QCryptographicHash::Keccak_256).toHex()).toUpper();
...
{
	auto nodeAccount =
		std::find_if(mData.begin(), mData.end(), [this](const NodeAccount& m) -> bool {
			return m.keyUid == m_selectedAccount;
		});
...
QVariantMap LoginModel::currentAccount() const
{
	...
	{
		if(mData[row].keyUid != m_selectedAccount)
			continue;
