...
void setDerivedKeys(GeneratedAccount& acc, const QJsonObject obj)
{
	acc.derivedKeys = {
		...
		{Constants::PathDefaultWallet,
		 {obj[Constants::PathDefaultWallet]["publicKey"].toString(), obj[Constants::PathDefaultWallet]["address"].toString()}}};
}
...
void storeDerivedAccount(QString& accountId, QString& hashedPassword)
{
	...
	const char* multiaccounstorederivedresult = MultiAccountStoreDerivedAccounts(Utils::jsonToStr(obj).toUtf8().data());
...
{
	return QJsonObject{{"name", Utils::generateAlias(account->derivedKeys[Constants::PathWhisper].publicKey)},
...
QJsonObject getAccountSettings(GeneratedAccount* account, QString installationId)
{
	...
	return QJsonObject{{"key-uid", account->keyUid},
					   ...
					   {"wallet-root-address", account->derivedKeys[Constants::PathWalletRoot].address},
...
{
	return QJsonArray{QJsonObject{{"public-key", account->derivedKeys[Constants::PathDefaultWallet].publicKey},
								  {"address", account->derivedKeys[Constants::PathDefaultWallet].address},
								  {"color", "#4360df"},
								  {"wallet", true},
								  {"path", Constants::PathDefaultWallet},
								  {"name", "Status account"}},
					  QJsonObject{{"public-key", account->derivedKeys[Constants::PathWhisper].publicKey},
...
bool saveAccountAndLogin(GeneratedAccount* genAccount, QString password)
{
	...
	SaveAccountAndLogin(Utils::jsonToStr(accountData).toUtf8().data(),
...
QString OnboardingModel::importMnemonic(QString mnemonic)
{
	...
	qCritical() << QJsonDocument::fromJson(importResult).object();
